 ...
 *
 * Electric flows:
 * - "electric-indexeddb": Electric SQL loaded from IndexedDB (persisted)
 * - "electric-memory": Electric SQL in-memory only
 * - "electric-api": Electric SQL fetched from API
 *
 * React Query flows:
 * - "rq-memory": React Query cache hit (in-memory)
 * - "rq-api": React Query fetched from API
 *
 * Legacy (for backwards compatibility):
 * - "memory", "indexeddb", "api", "unknown"
 */
export type DataFlow =
  | "electric-indexeddb"  // Electric SQL ↔ IndexedDB
  | "electric-memory"     // Electric SQL ↔ in-memory
  | "electric-api"        // Electric SQL ↔ API
  | "rq-memory"           // React Query ↔ in-memory cache
  | "rq-api"              // React Query ↔ API
...
function labelForSource(source: DataFlow): string {
  switch (source) {
    case "electric-indexeddb":
      return "Electric → IndexedDB";
